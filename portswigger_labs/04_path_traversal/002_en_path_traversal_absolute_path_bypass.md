...
> 🔗 Official Lab: https://portswigger.net/web-security/learning-paths/path-traversal/reading-arbitrary-files-via-path-traversal/file-path-traversal/lab-absolute-path-bypass  
...
## 🔬 Lab-specific observation

Traversal sequences such as `../` are blocked, but the application still accepts the absolute path `/etc/passwd`, allowing the filter to be bypassed.
